...
let db: any = null;
try {
  // Prefer environment variables (set in the hosting provider dashboard for production).
  ...
  // Fallback to the local config file when env vars are absent (local development).
  if (!firebaseConfig.projectId) {
    try {
      const configPath = path.join(process.cwd(), 'firebase-applet-config.json');
      Object.assign(firebaseConfig, JSON.parse(readFileSync(configPath, 'utf8')));
    } catch {
      // No on-disk config; Firebase-backed task persistence will remain disabled.
    }
  }
