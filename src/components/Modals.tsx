...
import { X, Search, Plus, Image as ImageIcon, FileCode, Layout, History, CheckCircle2, Clock, AlertCircle } from 'lucide-react';
    ...
    <Modal title="Asset Manager" onClose={() => setShowAssetManager(false)}>
      <div className="space-y-6">
        <div className="flex items-center gap-4 p-4 bg-white/5 rounded-2xl border border-white/5">
          ...
          <button className="ml-auto px-4 py-2 rounded-xl bg-nexus-accent text-black text-[10px] font-bold uppercase tracking-widest">Browse</button>
        </div>
...
export const TemplateSearch: React.FC = () => {
  ...
  return (
    <Modal title="Search Templates" onClose={() => setShowTemplateSearch(false)}>
      <div className="space-y-6">
        ...
        <div className="space-y-3">
          {[
            { name: 'E-commerce Dashboard', icon: Layout, category: 'Business' },
            { name: 'Real-time Chat Mesh', icon: FileCode, category: 'Social' },
            { name: 'Portfolio Portfolio', icon: ImageIcon, category: 'Creative' },
          ].map((t, i) => (
            <div key={i} className="flex items-center gap-4 p-4 bg-white/5 rounded-2xl border border-white/5 hover:border-nexus-accent/30 cursor-pointer transition-colors group">
              <div className="w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center group-hover:bg-nexus-accent/20 transition-colors">
                <t.icon size={20} className="text-white/40 group-hover:text-nexus-accent" />
              </div>
              <div>
                <h4 className="font-bold text-sm">{t.name}</h4>
                <p className="text-[10px] text-white/20 uppercase font-bold tracking-widest">{t.category}</p>
              </div>
              <Plus size={16} className="ml-auto text-white/20 group-hover:text-nexus-accent" />
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <Modal title="New Orchestration Task" onClose={() => setShowTaskModal(false)}>
      <div className="space-y-6">
        ...
        <button className="w-full py-4 rounded-2xl bg-nexus-accent text-black font-black uppercase tracking-widest shadow-[0_0_20px_rgba(0,240,255,0.3)] hover:scale-[1.02] transition-transform">Deploy Task</button>
      </div>
    </Modal>
  );
};
